...
public class Result {
    ...
    public Result(String pResult){
        int[] result = new int[gameData.getCombinationSize()];
        char[] charTemp = pResult.toCharArray();
    ...
    public void printResult(){
        if (gameData.getGameType() == GameType.MASTERMIND) {
        ...
        } else if (gameData.getGameType() == GameType.CODEX){
            ...
            for (int i=0; i<gameData.getCombinationSize();i++) {
                if(this.getResult()[i]==0){
                    ...
                    System.out.println("PIPPO an error has occurred");
                }
            }
            System.out.println();
        }
    ...
    public boolean isWinner(GameType gameType) {
        if (gameType == GameType.MASTERMIND) {
            return this.getResult()[0] == gameData.getCombinationSize();
